#' plot general multiplex network function (similar to temporal and spatial)
#'
#' @param networks network list
#' @param interlayer a data frame with `layer_from`,`node_from`,`layer_to`, `node_to`, and `weight` columns.
#' @param binary TRUE or FALSE: return a binary or weighted multiplex network
...
#'
#'
plot_multiplex <- function(networks, interlayer, binary = FALSE, y_adjust = 1,
  ...
  max_n_sp <- vector(length = n_layer)
  for (i in 1:n_layer) {
    max_n_sp[i] <- max(dim(networks[[i]]))
  }
  ...
  high_position <- list()
  for (i in 1:n_layer) {
    n_low <- nrow(networks[[i]]) # number of low level species
    n_high <- ncol(networks[[i]]) # number of high level species
    if (n_high > n_low){
  ...
  for (i in 1:n_layer) {
    polygon_data[[i]] <- data.frame(x = c(2*i-1-gap, 2*i + gap, 2*i + gap, 2*i-1-gap),
                                    y = c(-y_adjust, 1, (max(max_n_sp) + y_adjust + 1),
                                          max(max_n_sp)),
                                    layer = i)
  }
    ...
    if (is.null(extend)) {
      p <- ggplot() +
        ...
        geom_segment(data = interlayer_position %>% filter(weight > 0),
                     ...
                     linetype = inter_lty, alpha = 0.5) + # inter-links
    ...
    if (is.null(extend)) {
      p <- ggplot() +
        ...
        geom_segment(data = interlayer_position %>% filter(weight > 0),
                     ...
                     color = inter_lcol,
                     linetype = inter_lty, alpha = 0.25) + # inter-links
        geom_point(data = species_position %>% filter(strength > 0),
